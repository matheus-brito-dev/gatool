...
/*
 * 
 * Arquivo para orientação do genoma apenas no modo online
 * 
...
class Operacao
{
    ...
    public function orientationContigsUpload1()
    {
    
    }

     public function orientationContigsUpload2($f)

    {
         
        if($f == 2){ //caso spades com form online
        ...
        self::PARAMS2, self::REF_FASTA_PATH . $_SESSION['fileNameRefFasta'] . 
        self::PARAMS3, self::REF_GB_PATH . $_SESSION['fileNameRefGb'] .
       ...
       $files = scandir(self::ORIENTATION_PATH);
        /*var_dump($files[count($files) -1]); //doc
        var_dump($files[count($files) -2]); //unmapped
        var_dump($files[count($files) -3]);//mapped*/
       //
      ...
      //removendo arquivo de resultado do mapeamento
       ...
       //Chamando instancia ZIP para zipar o conteudo das pastas
        ...
        }else{ echo "Error in the genome orientation. Please verify the references files.";}
          
        
     }else if($f == 3){ //caso velvet com form online
         
            $file = Util::getFileName($_SESSION['fileName']);
            $file = $file . uniqid();
         
            //var_dump($_SESSION['fileNameFasta']);
            //var_dump($_SESSION['fileNameGb']);
